...
from utils import run_pdffigures2, TMLR_DIR, DB_PATH, SLUGIFY, FIG_RAG_DIR
...
# Figures destination
OUTPUT_BASE = os.path.join(TMLR_DIR, "figures")
...
def get_papers_metadata():
    """
    Query DuckDB to get the list of papers and their expected paths.
    """
...
def create_index():
    ...
    con = duckdb.connect(DB_PATH)
    con.execute("CREATE INDEX IF NOT EXISTS figures_index ON Figures(platform_id, figure_number)")
    con.close()
...
def main():
    ...
    for (year, decision), paper_list in tqdm(batch_map.items(), desc="Batches"):
        ...
        if db_entries:
            con.executemany("INSERT INTO Figures VALUES (?, ?, ?, ?, ?, ?, ?, ?)", db_entries)
