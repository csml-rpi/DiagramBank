...
- Use Papers_Staging (from latest icml-papers.jsonl) as the source of desired decisions.
- Do not move papers across years; keep the existing year folder on disk.

On-disk layouts (relative to FIG_RAG_DIR):
- papers:  OpenReview/ICML/papers/<year>/<decision>/<platform_id>/main.pdf
- figures: OpenReview/ICML/figures/<year>/<decision>/<platform_id>/Figure{n}.png
"""
...
from utils import ICML_DIR, DB_PATH, FIG_RAG_DIR

PAPERS_BASE = os.path.join(ICML_DIR, "papers")
FIGURES_BASE = os.path.join(ICML_DIR, "figures")
...
def main():
    ...
    con.execute(
        "CREATE OR REPLACE TABLE Papers_Staging AS SELECT * FROM read_json_auto('icml-papers.jsonl')"
    )
