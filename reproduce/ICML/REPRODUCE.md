# DiagramBank/ICML — Runbook (Manual Pipeline)
...
- DuckDB database lives at:
  - `${FIG_RAG_DIR}/OpenReview/ICML/research.db`
...
1) In `icml.py`, comment out old years and keep only the year(s) you want.
2) Run `python icml.py` to regenerate `icml-papers.jsonl` (staging snapshot).
