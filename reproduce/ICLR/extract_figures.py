...
from utils import run_pdffigures2, ICLR_DIR, DB_PATH, FIG_RAG_DIR
...
# Figures destination
OUTPUT_BASE = os.path.join(ICLR_DIR, "figures")
...
def get_papers_metadata():
    """Get papers (pid, year, decision_slug) from the latest scrape (Papers_Staging).

    We intentionally avoid filesystem scans (no os.walk) and use the canonical on-disk
    layout: papers/<year>/<decision_slug>/<pid>/main.pdf
    """
...
def create_index():
    """
    Index Figures on platform_id, figure_number.
    """
...
def main():
    ...
    # 2. Get Paper List from latest scrape (staging)
    print("Loading Papers_Staging from iclr-papers.jsonl...")
    ...
    for pid, year, decision in papers:
        ...
        if os.path.exists(pdf_path):
            key = (year, decision)
            batch_map.setdefault(key, []).append((pid, pdf_path))
