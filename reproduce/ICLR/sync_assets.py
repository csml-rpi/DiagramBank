...
- Use Papers_Staging (from latest iclr-papers.jsonl) as the source of desired decisions.
- Do not move papers across years; keep the existing year folder on disk.

On-disk layouts (relative to FIG_RAG_DIR):
- papers:  OpenReview/ICLR/papers/<year>/<decision>/<platform_id>/main.pdf
- figures: OpenReview/ICLR/figures/<year>/<decision>/<platform_id>/Figure{n}.png

Run:
  python3 sync_assets.py

Optional:
  python3 sync_assets.py --limit 100
  python3 sync_assets.py --dry_run
"""
...
from utils import ICLR_DIR, DB_PATH, FIG_RAG_DIR

PAPERS_BASE = os.path.join(ICLR_DIR, "papers")
FIGURES_BASE = os.path.join(ICLR_DIR, "figures")


def slugify_decision(decision: str) -> str:
    """Slugify decisions for on-disk paths.

    - lower -> non-alnum to '_' -> trim '_'
    - normalize pending/unknown to the folder name you use: pending_unknown
    """
...
def build_pending_indices():
    """Index only */pending_unknown/<pid>/ dirs for PDFs + figures.

    On-disk layout (canonical):
      papers/<year>/pending_unknown/<pid>/main.pdf
      figures/<year>/pending_unknown/<pid>/Figure*.png
    """
    print("⏳ Indexing pending_unknown assets...")

    pdf_index: dict[str, tuple[str, str]] = {}
    # map pid -> (dir, year)
    # Only walk <year>/pending_unknown subtrees
...
def update_figure_paths(con, pid: str, figure_dir: str) -> int:
    # Expected directory contents: Figure*.png only (canonical output of this pipeline).
    updates = []
...
def main():
    ...
    # Index only pending_unknown folders
    pdf_index, fig_index = build_pending_indices()

    con = duckdb.connect(DB_PATH)

    # Use staging as input (latest scrape)
    con.execute(
        "CREATE OR REPLACE TABLE Papers_Staging AS SELECT * FROM read_json_auto('iclr-papers.jsonl')"
    )
    ...
    # Candidates are only those that exist under pending_unknown on disk
    ...
    eligible = 0
    skipped_pdf_dest_exists = 0
    skipped_fig_dest_exists = 0
    ...
    for pid in tqdm(candidate_pids, desc="Syncing Assets", unit="paper"):
        ...
        # Move within the SAME year as currently on disk
        if pid in pdf_index:
            src_pdf_dir, year = pdf_index[pid]
            dst_pdf_dir = os.path.join(PAPERS_BASE, str(year), target_decision, pid)
            if os.path.exists(dst_pdf_dir):
                skipped_pdf_dest_exists += 1
            else:
        ...
        if pid in fig_index:
            ...
            if os.path.exists(dst_fig_dir):
                skipped_fig_dest_exists += 1
            else:
    ...
    print(f"Skipped (PDF dest exists): {skipped_pdf_dest_exists}")
    print(f"Skipped (Fig dest exists): {skipped_fig_dest_exists}")
